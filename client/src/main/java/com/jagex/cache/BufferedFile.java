...
public final class BufferedFile {
	...
	private void flush() throws IOException {
		...
		if (start > -1L && end > start) {
			...
			JString.copy(this.writeBuffer, (int) (start - this.writePosition), this.readBuffer, (int) (start - this.readPosition), copyLen);
		}
		this.writeLen = 0;
	...
	public void read(@OriginalArg(0) int off, @OriginalArg(1) byte[] b, @OriginalArg(2) int len) throws IOException {
		try {
			if (len > b.length) {
			...
			if (this.writePosition != -1L && this.virtualPosition >= this.writePosition && (long) this.writeLen + this.writePosition >= (long) len + this.virtualPosition) {
				JString.copy(this.writeBuffer, (int) (this.virtualPosition - this.writePosition), b, 0, len);
			...
			if (this.readPosition <= this.virtualPosition && this.readPosition + (long) this.readLen > this.virtualPosition) {
				...
				JString.copy(this.readBuffer, (int) (this.virtualPosition - this.readPosition), b, 0, n);
				off = n;
				len -= n;
			...
			if (this.readBuffer.length < len) {
				...
				JString.copy(this.readBuffer, 0, b, off, n);
				len -= n;
				off += n;
				this.virtualPosition += n;
			}
			if (this.writePosition != -1L) {
				...
				if (end > -1L && start > end) {
					@Pc(426) int local426 = (int) (start - end);
					JString.copy(this.writeBuffer, (int) (end - this.writePosition), b, (int) (end - originalPosition), local426);
		...
		} catch (@Pc(464) IOException exception) {
	...
	public void write(@OriginalArg(0) byte[] b, @OriginalArg(1) int off, @OriginalArg(3) int len) throws IOException {
		try {
			...
			if (this.writePosition != -1L && (long) this.writeBuffer.length + this.writePosition < (long) len + this.virtualPosition) {
				...
				len -= n;
				JString.copy(b, off, this.writeBuffer, (int) (this.virtualPosition - this.writePosition), n);
				this.virtualPosition += n;
			...
			if (this.writeBuffer.length < len) {
				...
				if (start > -1L && end > start) {
					...
					JString.copy(b, (int) (start + (long) off - this.virtualPosition), this.readBuffer, (int) (start - this.readPosition), copyLen);
				}
				this.virtualPosition += len;
			} else if (len > 0) {
				if (this.writePosition == -1L) {
				...
				JString.copy(b, off, this.writeBuffer, (int) (this.virtualPosition - this.writePosition), len);
				this.virtualPosition += len;
	...
	public void seek(@OriginalArg(1) long offset) throws IOException {
		if (offset < 0L) {
