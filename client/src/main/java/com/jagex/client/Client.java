...
import com.jagex.network.LoginProt;
import com.jagex.sign.SignedResourceStatus;
import com.jagex.sound.core.MixerPcmStream;
...
import com.jagex.client.ui.LoadingBar;
import com.jagex.client.ui.LoadingBarAwt;
import com.jagex.client.ui.TitleScreen;
...
import com.jagex.game.logic.HintArrowManager;
import com.jagex.game.PathFinder;
import com.jagex.game.combat.HitBarList;
import com.jagex.game.inventory.Equipment;
...
import com.jagex.sound.AudioChannel;
import com.jagex.sound.midi.MidiPcmStream;
import com.jagex.sound.midi.MidiPlayer;
import com.jagex.sound.pcm.PcmResampler;
import com.jagex.cache.BufferedFile;
...
import com.jagex.types.varbit.VarBitTypeList;
import com.jagex.types.varp.VarpTypeList;
...
public final class Client extends GameShell {
	...
	public static final LruCache cache = new LruCache(4);
	...
	public static int modeWhere = ModeWhere.LIVE;
	...
	public static SignedResource js5SocketRequest;
	...
	public static void method2721() {
		...
		if (gameState == 10) {
			InterfaceManager.initializeLoginScreen(false);
		}
		if (gameState == 30) {
	...
	public static void method4221() {
		...
		LocChangeRequest.queue = new LinkedList();
		FriendList.state = 0;
...
//	}
	...
	protected final void mainRedraw() {
		...
		if ((gameState == 30 || gameState == 10) && (GameShell.canvasReplaceRecommended || DisplayMode.aLong89 != 0L && DisplayMode.aLong89 < SystemTimer.safetime())) {
		...
		if (GameShell.fullScreenFrame == null) {
			...
			if (local80 != GameShell.frameWidth || local84 != GameShell.frameHeight) {
				...
				DisplayMode.aLong89 = SystemTimer.safetime() + 500L;
			}
		}
		...
		if (GlRenderer.enabled) {
			for (local80 = 0; local80 < 100; local80++) {
				InterfaceManager.componentNeedsRedraw[local80] = true;
			}
		}
		if (gameState == 0) {
		...
		if (GlRenderer.enabled && gameState != 0) {
			...
			if ((gameState == 30 || gameState == 10) && Cheat.rectDebug == 0 && !local158) {
				try {
					...
					for (local84 = 0; local84 < InterfaceManager.rectangles; local84++) {
						if (InterfaceManager.rectangleRedraw[local84]) {
							SoftwareRenderer.frameBuffer.drawAt(InterfaceManager.rectangleWidth[local84], InterfaceManager.rectangleX[local84], InterfaceManager.rectangleHeight[local84], local388, InterfaceManager.rectangleY[local84]);
							InterfaceManager.rectangleRedraw[local84] = false;
						}
					}
		...
		if (Preferences.safeMode && gameState == 10 && InterfaceManager.topLevelInterface != -1) {
...
//	}
	...
	protected void mainInit() {
		...
		if ((SignLink.anInt5928 == 3 && modeWhere != ModeWhere.WIP)) {
	...
	private void mainUpdate() {
		for (InterfaceManager.keyQueueSize = 0; Keyboard.nextKey() && InterfaceManager.keyQueueSize < 128; InterfaceManager.keyQueueSize++) {
			InterfaceManager.keyCodes[InterfaceManager.keyQueueSize] = Keyboard.keyCode;
			InterfaceManager.keyChars[InterfaceManager.keyQueueSize] = Keyboard.keyChar;
		}
		Protocol.sceneDelta++;
		if (InterfaceManager.topLevelInterface != -1) {
			InterfaceManager.renderInterface(0, 0, 0, GameShell.canvasWidth, InterfaceManager.topLevelInterface, 0, GameShell.canvasHeight);
		}
		InterfaceManager.transmitTimer++;
		if (GlRenderer.enabled) {
				...
				if (npcEntity != null) {
					...
					if ((walkflags & 0x2) > 0 && npcEntity.movementQueueSize == 0 && Math.random() * 1000.0D < 10.0D) {
						...
						if (local98 != 0 || local106 != 0) {
							...
							PathFinder.collisionMaps[Player.currentLevel].unflagLoc(npcEntity.xFine >> 7, npcEntity.getSize(), false, 0, npcEntity.getSize(), npcEntity.zFine >> 7);
...
//											}
	...
	private void js5Connect() {
		if (js5PrevErrors < js5NetQueue.errors) {
			...
			if (js5NetQueue.errors >= 2 && js5NetQueue.response == Js5ResponseCode.CONNECT_OUTOFDATE) {
				this.error("js5connect_outofdate");
				gameState = 1000;
				return;
			}
			if (js5NetQueue.errors >= 4 && js5NetQueue.response == Js5ResponseCode.DISCONNECTED) {
				this.error("js5crc");
				gameState = 1000;
				return;
			}
			if (js5NetQueue.errors >= 4 && (gameState == 0 || gameState == 5)) {
				if (js5NetQueue.response == Js5ResponseCode.CONNECT_FULL1 || js5NetQueue.response == Js5ResponseCode.CONNECT_FULL2) {
		...
		try {
			if (js5ConnectState == Js5State.INIT) {
				js5SocketRequest = GameShell.signLink.openSocket(hostname, port);
				js5ConnectState++;
			}
			if (js5ConnectState == Js5State.WAITING_CONNECTION_OPENED) {
				if (js5SocketRequest.status == SignedResourceStatus.ERROR) {
					this.setJs5Response(1000);
					return;
				}
				if (js5SocketRequest.status == SignedResourceStatus.SUCCESS) {
					js5ConnectState++;
				}
			}
			if (js5ConnectState == Js5State.SEND_HEADER) {
				js5Socket = new BufferedSocket((Socket) js5SocketRequest.result, GameShell.signLink);
				@Pc(194) Packet packet = new Packet(5);
				packet.p1(LoginProt.INIT_JS5REMOTE_CONNECTION);
				packet.p4(530); //  Client Rev
				js5Socket.write(5, packet.data);
				js5ConnectState++;
				js5ConnectTime = SystemTimer.safetime();
			}
			if (js5ConnectState == Js5State.WAITING_FIRST_RESPONSE) {
				if (gameState == 0 || gameState == 5 || js5Socket.available() > 0) {
					...
					js5ConnectState++;
				} else if (SystemTimer.safetime() - js5ConnectTime > 30000L) {
					this.setJs5Response(1001);
					return;
				}
			}
			if (js5ConnectState == Js5State.WAITING_LOADING_REQUIREMENTS) {
				@Pc(296) boolean loggedOut = gameState == 5 || gameState == 10 || gameState == 28; // TODO move to constants
				js5NetQueue.start(!loggedOut, js5Socket);
				js5Socket = null;
				js5SocketRequest = null;
				js5ConnectState = Js5State.INIT;
			}
	...
	private void mainLoad() {
		...
		if (mainLoadState == 10) {
			...
			if (js5MasterIndex.isReady()) {
				...
				CLIENTSCRIPTS = createJs5(false, true, true, 12);
			...
			if (percentage >= 1100) {
				...
				InterfaceManager.init(js5Archive13, js5Archive8, js5Archive3, js5Archive7);
			...
			} else if (!CLIENTSCRIPTS.fetchAll()) {
				mainLoadSecondaryText = JString.concatenate(new JString[] { LocalizedText.MAINLOAD130, JString.parseInt(CLIENTSCRIPTS.getPercentageComplete() / 10 + 75), JString.PERCENT_SIGN});
				mainLoadPercentage = 85;
