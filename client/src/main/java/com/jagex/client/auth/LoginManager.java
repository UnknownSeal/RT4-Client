...
public class LoginManager {
    ...
    public static void method1841() {
        ...
        for (componentIndex = 0; componentIndex < InterfaceManager.rectangles; componentIndex++) {
            if (InterfaceManager.componentNeedsRedraw[componentIndex]) {
                InterfaceManager.rectangleRedraw[componentIndex] = true;
            }
            InterfaceManager.componentRedrawPrevious[componentIndex] = InterfaceManager.componentNeedsRedraw[componentIndex];
            InterfaceManager.componentNeedsRedraw[componentIndex] = false;
        }
        hoveredComponent = null;
        ClientScriptRunner.anInt2503 = -1;
        InterfaceManager.anInt5574 = -1;
        InterfaceManager.mouseOverInventoryComponent = null;
        if (GlRenderer.enabled) {
            ClientScriptRunner.aBoolean299 = true;
        }
        InterfaceManager.currentFrameLoop = Client.loop;
        if (InterfaceManager.topLevelInterface != -1) {
            InterfaceManager.rectangles = 0;
        ...
        InterfaceManager.setCursor(componentIndex);
        ...
        if (Cheat.rectDebug == 3) {
            for (@Pc(189) int rectIndex = 0; rectIndex < InterfaceManager.rectangles; rectIndex++) {
                if (InterfaceManager.componentRedrawPrevious[rectIndex]) {
                    if (GlRenderer.enabled) {
                        GlRaster.fillRectAlpha(InterfaceManager.rectangleX[rectIndex], InterfaceManager.rectangleY[rectIndex], InterfaceManager.rectangleWidth[rectIndex], InterfaceManager.rectangleHeight[rectIndex], 16711935, 128);
                    } else {
                        SoftwareRenderer.fillRectAlpha(InterfaceManager.rectangleX[rectIndex], InterfaceManager.rectangleY[rectIndex], InterfaceManager.rectangleWidth[rectIndex], InterfaceManager.rectangleHeight[rectIndex], 16711935, 128);
                    }
                } else if (InterfaceManager.rectangleRedraw[rectIndex]) {
                    if (GlRenderer.enabled) {
                        GlRaster.fillRectAlpha(InterfaceManager.rectangleX[rectIndex], InterfaceManager.rectangleY[rectIndex], InterfaceManager.rectangleWidth[rectIndex], InterfaceManager.rectangleHeight[rectIndex], 16711680, 128);
                    } else {
                        SoftwareRenderer.fillRectAlpha(InterfaceManager.rectangleX[rectIndex], InterfaceManager.rectangleY[rectIndex], InterfaceManager.rectangleWidth[rectIndex], InterfaceManager.rectangleHeight[rectIndex], 16711680, 128);
                    }
                }
            }
        }
    ...
    public static void loopAuto() {
        ...
        try {
            ...
            if (autoStep == LoginStep.CONNECT) {
                Protocol.socketRequest = GameShell.signLink.openSocket(Client.worldListHostname, Client.worldListPort);
                autoStep = LoginStep.WAIT_FOR_CONNECTION_ACK;
            }
            @Pc(126) int worldId;
            if (autoStep == LoginStep.WAIT_FOR_CONNECTION_ACK) {
                if (Protocol.socketRequest.status == SignedResourceStatus.ERROR) {
                    throw new IOException();
                }
                if (Protocol.socketRequest.status != SignedResourceStatus.SUCCESS) {
                    return;
                }
                ...
                worldId = Protocol.gameServerSocket.read();
                if (Client.musicChannel != null) {
                    Client.musicChannel.skipConsumptionCheck();
                }
                if (Client.soundChannel != null) {
                ...
                autoStep = LoginStep.SEND_LOGIN_PACKET;
            }
            if (autoStep == LoginStep.SEND_LOGIN_PACKET) {
                if (Protocol.gameServerSocket.available() < 2) {
                    return;
                }
                worldId = Protocol.gameServerSocket.read() << 8 | Protocol.gameServerSocket.read();
                WorldList.hopWorld(worldId);
                if (Player.worldId == -1) {
                    autoStep = LoginStep.DELAY;
                    reply = 6;
                    Protocol.gameServerSocket.closeGracefully();
                    Protocol.gameServerSocket = null;
                    return;
            ...
            if (connectionRetryCount < 1) {
                ...
                reply = LoginResponseCode.ERROR_CONNECTING_TO_SERVER;
                autoStep = LoginStep.DELAY;
            }
        }
    }
    ...
    public static void loop() {
        ...
        try {
            ...
            if (step == LoginStep.SEND_LOGIN_PACKET) {
                ...
                packet.p4(Client.CLIENTSCRIPTS.getChecksum());
            ...
            if (step == LoginStep.WAIT_FOR_LOGIN_RESPONSE) {
                ...
                @Pc(623) int responseCode = Protocol.gameServerSocket.read();
                if (responseCode == LoginResponseCode.HOP_BLOCKED) {
                    step = LoginStep.WAIT_FOR_HOP_BLOCK_DURATION;
                } else if (responseCode == LoginResponseCode.DISALLOWED_BY_SCRIPT) {
                    ...
                    reply = responseCode;
                    return;
                } else if (responseCode == LoginResponseCode.LOGIN_SERVER_NO_REPLY && errors < 1) {
            ...
            if (step == LoginStep.WAIT_FOR_HOP_BLOCK_DURATION) {
                if (Protocol.gameServerSocket.available() >= 1) {
                    hopTime = (Protocol.gameServerSocket.read() + 3) * 60;
                    step = LoginStep.DELAY;
                    reply = LoginResponseCode.HOP_BLOCKED;
                    Protocol.gameServerSocket.closeGracefully();
                    Protocol.gameServerSocket = null;
                    return;
                }
            ...
            if (step == LoginStep.WAIT_FOR_SCRIPT_DISALLOW_REASON) {
                if (Protocol.gameServerSocket.available() >= 1) {
                    disallowResult = Protocol.gameServerSocket.read();
                    step = LoginStep.DELAY;
                    reply = LoginResponseCode.DISALLOWED_BY_SCRIPT;
                    Protocol.gameServerSocket.closeGracefully();
                    Protocol.gameServerSocket = null;
                    return;
                }
            ...
            if (step == LoginStep.WAIT_FOR_LOGIN_OK_LENGTH) {
