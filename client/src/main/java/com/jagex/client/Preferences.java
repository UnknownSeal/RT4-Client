...
public class Preferences {
    ...
    public static void save(@OriginalArg(0) SignLink signLink) {
        @Pc(11) FileOnDisk preferencesFile = null;
        try {
            @Pc(16) SignedResource openRequest = signLink.openPreferences("runescape");
            while (openRequest.status == SignedResourceStatus.IDLE) {
                ThreadUtils.sleep(1L);
            }
            if (openRequest.status == SignedResourceStatus.SUCCESS) {
    ...
    public static void read(@OriginalArg(0) SignLink signLink) {
        ...
        favoriteWorlds = 0;
        try {
            @Pc(78) SignedResource openRequest = signLink.openPreferences("runescape");
            while (openRequest.status == SignedResourceStatus.IDLE) {
                ThreadUtils.sleep(1L);
            }
            if (openRequest.status == SignedResourceStatus.SUCCESS) {
