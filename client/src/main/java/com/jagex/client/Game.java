...
public class Game {
    ...
    public static void updateGame() {
        ...
        if (Preferences.keyboardCameraEnabled) {
            for (i = 0; i < InterfaceManager.keyQueueSize; i++) {
                packetStartOffset = InterfaceManager.keyCodes[i];
        ...
        for (@Pc(709) DelayedStateChange change = DelayedStateChange.removeFirst(); change != null; change = DelayedStateChange.removeFirst()) {
            ...
            if (mouseSampleCount == 1) {
                ...
                if (mouseSampleCount == 3) {
                    ...
                    if (component.modelSeqId != change.secondaryData || change.secondaryData == -1) {
                        ...
                        component.modelSeqId = change.secondaryData;
                        component.animationId = 0;
                        InterfaceManager.redraw(component);
                    }
                } else if (mouseSampleCount == 6) {
                    y = change.secondaryData;
                    x = y >> 10 & 0x1F;
                    dx = y & 0x1F;
                    ...
                    if (component.modelXOffset != change.secondaryData || change.primaryData != component.modelZOffset || component.modelYOffset != change.tertiaryData) {
                        ...
                        component.modelXOffset = change.secondaryData;
                        InterfaceManager.redraw(component);
                    }
        ...
        if (MiniMenu.pressedInventoryComponent != null) {
            ...
            if (MiniMenu.inventoryPressTimer >= 15) {
                InterfaceManager.redraw(MiniMenu.pressedInventoryComponent);
        ...
        if (InterfaceManager.clickedInventoryComponent != null) {
            ...
            if (InterfaceManager.clickedInventoryComponentX + 5 < Mouse.lastMouseX || Mouse.lastMouseX < InterfaceManager.clickedInventoryComponentX - 5 || InterfaceManager.clickedInventoryComponentY + 5 < Mouse.lastMouseY || InterfaceManager.clickedInventoryComponentY - 5 > Mouse.lastMouseY) {
                InterfaceManager.draggingClickedInventoryObject = true;
            }
            InterfaceManager.lastItemDragTime++;
            if (Mouse.pressedButton == 0) { // Mouse released
                if (InterfaceManager.draggingClickedInventoryObject && InterfaceManager.lastItemDragTime >= 5) {
                    ...
                    if (InterfaceManager.clickedInventoryComponent == InterfaceManager.mouseOverInventoryComponent && InterfaceManager.selectedInventorySlot != MiniMenu.mouseInvInterfaceIndex) {
                        ...
                        if (VarpDomain.bankInsertMode == 1 && component.clientcode == 206) {
                            insertMode = 1;
                        }
                ...
                InterfaceManager.clickedInventoryComponent = null;
            }
        }
        ...
        InterfaceManager.previousMouseOverComponent = null;
        @Pc(1508) Component local1508 = Protocol.dragHoverComponent;
        Protocol.dragHoverComponent = null;
        while (Keyboard.nextKey() && InterfaceManager.keyQueueSize < 128) {
                                ...
                                while (true) {
                                    do {
                                        ...
                                        if (highPriorityRequest == null) {
                                            ...
                                            if (InterfaceManager.previousMouseOverComponent != component) {
                                                if (component != null) {
                                                    InterfaceManager.redraw(component);
                                                }
                                                if (InterfaceManager.previousMouseOverComponent != null) {
                                                    InterfaceManager.redraw(InterfaceManager.previousMouseOverComponent);
                                                }
                                            }
                                            if (local1508 != Protocol.dragHoverComponent && ClientScriptRunner.MAX_CALL_STACK_DEPTH == Protocol.dragHoverAnimationProgress) {
                                                if (local1508 != null) {
                                                    InterfaceManager.redraw(local1508);
                                                }
                                                if (Protocol.dragHoverComponent != null) {
                                                    InterfaceManager.redraw(Protocol.dragHoverComponent);
                                                }
                                            }
                                            if (Protocol.dragHoverComponent == null) {
                                            ...
                                            } else if (Camera.mode == MODE_ORBITAL) {
