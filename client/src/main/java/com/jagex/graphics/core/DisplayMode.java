package com.jagex.graphics.core;
...
public final class DisplayMode {
	...
	public static DisplayMode[] getModesInternal(@OriginalArg(1) SignLink signLink) {
		...
		@Pc(17) SignedResource request = signLink.getDisplayModes();
		while (request.status == SignedResourceStatus.IDLE) {
			ThreadUtils.sleep(10L);
		}
		if (request.status == SignedResourceStatus.ERROR) {
			return new DisplayMode[0];
		}
	...
	public static Frame enterFullScreen(@OriginalArg(2) int bitDepth, @OriginalArg(3) int height, @OriginalArg(4) int width, @OriginalArg(5) SignLink signLink) {
		...
		@Pc(90) SignedResource request = signLink.enterFullScreen(bitDepth, height, width);
		while (request.status == SignedResourceStatus.IDLE) {
			ThreadUtils.sleep(10L);
		}
		...
		if (frame == null) {
			return null;
		} else if (request.status == SignedResourceStatus.ERROR) {
	...
	public static void exitFullScreen(@OriginalArg(0) Frame frame, @OriginalArg(2) SignLink signLink) {
		while (true) {
			@Pc(16) SignedResource request = signLink.exitFullScreen(frame);
			while (request.status == SignedResourceStatus.IDLE) {
				ThreadUtils.sleep(10L);
			}
			if (request.status == SignedResourceStatus.SUCCESS) {
	...
	public static void setWindowMode(@OriginalArg(0) boolean replaceCanvas, @OriginalArg(1) int mode, @OriginalArg(2) boolean arg2, @OriginalArg(3) int windowMode, @OriginalArg(5) int width, @OriginalArg(6) int height) {
		...
		if (arg2 && mode > 0) {
			...
			if (!aBoolean73) {
				...
				@Pc(300) SignedResource request = GameShell.signLink.loadGlNatives(Client.instance.getClass());
				while (request.status == SignedResourceStatus.IDLE) {
					ThreadUtils.sleep(100L);
				}
				if (request.status == SignedResourceStatus.SUCCESS) {
		...
		for (@Pc(466) int local466 = 0; local466 < 100; local466++) {
			InterfaceManager.componentNeedsRedraw[local466] = true;
		}
