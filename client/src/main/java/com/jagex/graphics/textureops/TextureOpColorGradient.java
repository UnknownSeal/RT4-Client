...
public final class TextureOpColorGradient extends TextureOp {
	...
	public void decode(@OriginalArg(1) Packet packet, @OriginalArg(0) int op) {
		...
		this.samples = new int[packet.g1()][4];
		for (@Pc(35) int local35 = 0; local35 < this.samples.length; local35++) {
			this.samples[local35][0] = packet.g2();
			this.samples[local35][1] = packet.g1() << 4;
			this.samples[local35][2] = packet.g1() << 4;
			this.samples[local35][3] = packet.g1() << 4;
		}
	}
