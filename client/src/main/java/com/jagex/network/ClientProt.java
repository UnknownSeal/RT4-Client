...
public class ClientProt {
    ...
    public static void method3502(@OriginalArg(1) int length, @OriginalArg(2) int type) {
        ...
        length--;
        @Pc(23) int startX = PathFinder.queueX[length];
        @Pc(27) int startZ = PathFinder.queueZ[length];
        ...
        LoginManager.flagSceneTileX = PathFinder.queueX[0];
        LoginManager.flagSceneTileZ = PathFinder.queueZ[0];

        for (@Pc(126) int i = 1; i < bufferSize; i++) {
            length--;
            Protocol.outboundBuffer.p1b_alt1(PathFinder.queueX[length] - startX);
            Protocol.outboundBuffer.p1b_alt3(PathFinder.queueZ[length] - startZ);
        }
    }
    ...
    public static void clickPlayerOption(@OriginalArg(0) int type, @OriginalArg(2) JString arg1) {
        ...
        if (!local13) {
            ChatHistory.addMessage(JString.EMPTY, 0, JString.concatenate(new JString[] { LocalizedText.UNABLETOFIND, local7 }));
        }
    }
