package com.jagex.network.security;
...
public final class ReflectionCheck extends Node {
	...
	public static LinkedList queue = new LinkedList(); // Queue of pending reflection check requests from the server
	...
	public SignedResource[] functionNodes;
	...
	public SignedResource[] fieldRequests;
	...
	public static void push(@OriginalArg(0) SignLink arg0, @OriginalArg(1) Packet packet, @OriginalArg(2) int arg2) {
		...
		check.id = packet.g4();
		check.functionNodes = new SignedResource[check.size];
		check.errorCodes = new int[check.size];
		check.methodArguments = new byte[check.size][][];
		check.fieldRequests = new SignedResource[check.size];
		...
		for (@Pc(59) int i = 0; i < check.size; i++) {
			try {
				...
				if (opcode == 0 || opcode == 1 || opcode == 2) {
					className = new String(packet.gjstr().encode());
					valueOrArgCount = 0;
					memberName = new String(packet.gjstr().encode());
					if (opcode == 1) {
					...
					for (@Pc(173) int local173 = 0; local173 < valueOrArgCount; local173++) {
						parameterTypeNames[local173] = new String(packet.gjstr().encode());
					}
