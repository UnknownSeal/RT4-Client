package com.jagex.core.utils.debug;
...
import static com.jagex.network.ClientProt.CLIENT_CHEAT;
...
public class Cheat {
    ...
    public static void teleport(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) {
        @Pc(66) JString command = JString.concatenate(new JString[] {TELE, JString.parseInt(arg2), JString.COMMA_SEPARATOR, JString.parseInt(arg0 >> 6), JString.COMMA_SEPARATOR, JString.parseInt(arg1 >> 6), JString.COMMA_SEPARATOR, JString.parseInt(arg0 & 0x3F), JString.COMMA_SEPARATOR, JString.parseInt(arg1 & 0x3F) });
    ...
    public static void execute(@OriginalArg(0) JString chatTyped) {
        if (LoginManager.staffModLevel >= 2) {
            ...
            if (chatTyped.equalsIgnoreCase(GC)) {
                ...
                ChatHistory.addMessage(null, 0, JString.concatenate(new JString[] {aClass100_892, JString.parseInt(local38), DEBUG_MEM_UNIT}));
            }
            @Pc(117) int local117;
            if (chatTyped.equalsIgnoreCase(MM)) {
                ...
                local29 = Runtime.getRuntime();
                local38 = (int) ((local29.totalMemory() - local29.freeMemory()) / 1024L);
                ChatHistory.addMessage(null, 0, JString.concatenate(new JString[] {aClass100_893, JString.parseInt(local38), DEBUG_MEM_UNIT}));
                ...
                local38 = (int) ((local29.totalMemory() - local29.freeMemory()) / 1024L);
                ChatHistory.addMessage(null, 0, JString.concatenate(new JString[] {aClass100_1093, JString.parseInt(local38), DEBUG_MEM_UNIT}));
            }
            if (chatTyped.equalsIgnoreCase(PCACHESIZE)) {
                ChatHistory.addMessage(null, 0, JString.concatenate(new JString[] {aClass100_335, JString.parseInt(PlayerAppearance.getModelCacheSize()) }));
            }
            ...
            if (chatTyped.startsWith(COMMAND_FPS) && Client.modeWhere != ModeWhere.LIVE) {
            ...
            if (chatTyped.startsWith(RECT_DEBUG)) {
                ...
                ChatHistory.addMessage(null, 0, JString.concatenate(new JString[] {RECT_DEBUG_EQUALS, JString.parseInt(rectDebug) }));
            }
            ...
            if (chatTyped.equalsIgnoreCase(TWEEN)) {
                if (SeqType.applyTweening) {
                    ...
                    ChatHistory.addMessage(null, 0, FORCED_TWEENING_DISABLED);
                } else {
                    SeqType.applyTweening = true;
                    ChatHistory.addMessage(null, 0, FORCED_TWEENING_ENABLED);
                }
            }
        ...
        Protocol.outboundBuffer.pIsaac1(CLIENT_CHEAT);
