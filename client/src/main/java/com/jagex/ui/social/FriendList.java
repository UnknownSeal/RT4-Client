...
public class FriendList {
    ...
    public static void addFriend(@OriginalArg(0) long username) {
        ...
        if (friendCount >= 100 && !LoginManager.playerMember || friendCount >= 200) {
            ChatHistory.addMessage(JString.EMPTY, 0, LocalizedText.FRIENDLISTFULL);
            return;
        }
        ...
        for (i = 0; i < friendCount; i++) {
            if (encodedUsernames[i] == username) {
                ChatHistory.addMessage(JString.EMPTY, 0, JString.concatenate(new JString[] { displayName, LocalizedText.FRIENDLISTDUPE}));
                return;
            }
        }
        for (i = 0; i < IgnoreList.ignoreCount; i++) {
            if (username == IgnoreList.encodedIgnores[i]) {
                ChatHistory.addMessage(JString.EMPTY, 0, JString.concatenate(new JString[] { LocalizedText.REMOVESOCIAL1, displayName, LocalizedText.REMOVEIGNORE}));
                return;
            }
        }
        if (displayName.strEquals(PlayerList.self.username)) {
            ChatHistory.addMessage(JString.EMPTY, 0, LocalizedText.FRIENDCANTADDSELF);
            return;
        }
