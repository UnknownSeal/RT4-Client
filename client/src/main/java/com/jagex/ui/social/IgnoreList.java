...
import static com.jagex.network.ClientProt.IGNORELIST_ADD;
import static com.jagex.network.ClientProt.IGNORELIST_DEL;
...
public class IgnoreList {
    ...
    public static void addIgnore(@OriginalArg(1) long username) {
        ...
        if (ignoreCount >= 100) {
            ChatHistory.addMessage(JString.EMPTY, 0, LocalizedText.IGNORELISTFULL);
            return;
        }
        ...
        for (i = 0; i < ignoreCount; i++) {
            if (encodedIgnores[i] == username) {
                ChatHistory.addMessage(JString.EMPTY, 0, JString.concatenate(new JString[] { displayName, LocalizedText.IGNORELISTDUPE}));
                return;
            }
        }
        for (i = 0; i < FriendList.friendCount; i++) {
            if (FriendList.encodedUsernames[i] == username) {
                ChatHistory.addMessage(JString.EMPTY, 0, JString.concatenate(new JString[] { LocalizedText.REMOVESOCIAL2, displayName, LocalizedText.REMOVEFRIEND}));
                return;
            }
        }
        if (displayName.strEquals(PlayerList.self.username)) {
            ChatHistory.addMessage(JString.EMPTY, 0, LocalizedText.IGNORECANTADDSELF);
            return;
        }
