...
import com.jagex.graphics.gl.GlIndexedSprite;
...
public class SpriteLoader {
    ...
    public static void decode(@OriginalArg(0) byte[] arg0) {
        @Pc(4) Packet packet = new Packet(arg0);
        packet.offset = arg0.length - 2;
        frames = packet.g2();
        ...
        packet.offset = arg0.length - frames * 8 - 7;
        width = packet.g2();
        height = packet.g2();
        @Pc(66) int paletteSize = (packet.g1() & 0xFF) + 1;
        @Pc(68) int i;
        for (i = 0; i < frames; i++) {
            xOffsets[i] = packet.g2();
        }
        for (i = 0; i < frames; i++) {
            yOffsets[i] = packet.g2();
        }
        for (i = 0; i < frames; i++) {
            innerWidths[i] = packet.g2();
        }
        for (i = 0; i < frames; i++) {
            innerHeights[i] = packet.g2();
        }
        packet.offset = arg0.length + 3 - frames * 8 - paletteSize * 3 - 7;
        ...
        for (i = 0; i < frames; i++) {
            ...
            alpha[i] = local215;
            @Pc(223) int local223 = packet.g1();
            @Pc(232) int local232;
            if ((local223 & 0x1) == 0) {
                for (local232 = 0; local232 < local203; local232++) {
                    local206[local232] = packet.g1s();
                }
                if ((local223 & 0x2) != 0) {
                    for (local232 = 0; local232 < local203; local232++) {
                        @Pc(343) byte local343 = local215[local232] = packet.g1s();
                        local208 |= local343 != -1;
                    }
                }
            } else {
                ...
                label88: while (true) {
                    ...
                    if (local232 >= local195) {
                        ...
                        while (true) {
                            ...
                            for (local241 = 0; local241 < local199; local241++) {
                                @Pc(291) byte local291 = local215[local195 * local241 + local232] = packet.g1s();
                                local208 |= local291 != -1;
                            }
                            local232++;
                        }
                    ...
                    for (local241 = 0; local241 < local199; local241++) {
                        local206[local232 + local241 * local195] = packet.g1s();
                    }
                    local232++;
                }
            }
