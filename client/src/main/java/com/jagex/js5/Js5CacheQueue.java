...
public final class Js5CacheQueue implements Runnable {
	...
	public Js5CacheQueue() {
		@Pc(20) SignedResource request = GameShell.signLink.startThread(5, this);
	...
	public final Js5CacheRequest readSynchronous(@OriginalArg(0) Cache arg0, @OriginalArg(2) int arg1) {
		...
		local9.type = 1;
		@Pc(16) Queue local16 = this.queue;
		synchronized (this.queue) {
			@Pc(31) Js5CacheRequest local31 = (Js5CacheRequest) this.queue.first();
			while (true) {
				if (local31 == null) {
					break;
				}
				if (local31.key2 == (long) arg1 && local31.cache == arg0 && local31.type == 2) {
	...
	public final void run() {
		while (!this.stop) {
			@Pc(12) Queue local12 = this.queue;
			@Pc(19) Js5CacheRequest local19;
			synchronized (this.queue) {
				local19 = (Js5CacheRequest) this.queue.removeFirst();
				if (local19 == null) {
					try {
						this.queue.wait();
			...
			try {
				if (local19.type == 2) {
					local19.cache.put((int) local19.key2, local19.bytes.length, local19.bytes);
				} else if (local19.type == 3) {
					local19.bytes = local19.cache.get((int) local19.key2);
				}
