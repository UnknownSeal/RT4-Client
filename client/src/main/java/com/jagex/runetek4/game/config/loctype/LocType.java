...
public final class LocType {
	...
	private void decode(@OriginalArg(0) Packet packet, @OriginalArg(1) int code) {
		...
		if (code == 1) {
			...
			this.seqindex = packet.g2();
			if (this.seqindex == 65535) {
				this.seqindex = -1;
			}
		} else if (code == 27) {
			this.blockwalk = 1;
	...
	public boolean method3416(@OriginalArg(1) int arg0) {
		if (this.models != null) {
			for (@Pc(18) int index = 0; index < this.models.length; index++) {
				if (arg0 == this.models[index]) {
					return Static121.aClass153_45.requestDownload(this.shapes[index] & 0xFFFF, 0);
				}
			}
			return true;
			...
			for (@Pc(73) int index = 0; index < this.shapes.length; index++) {
				local71 &= Static121.aClass153_45.requestDownload(this.shapes[index] & 0xFFFF, 0);
			}
			return local71;
		} else {
			return true;
