...
public final class IDKType {
	...
	public ModelUnlit getHeadModel() {
		@Pc(13) int local13 = 0;
		@Pc(16) ModelUnlit[] modelUnlits = new ModelUnlit[5];
		for (@Pc(18) int index = 0; index < 5; index++) {
			if (this.heads[index] != -1) {
				modelUnlits[local13++] = ModelUnlit.get(Static14.aClass153_8, this.heads[index]);
			}
		}
		@Pc(52) ModelUnlit head = new ModelUnlit(modelUnlits, local13);
	...
	public ModelUnlit getModel() {
		...
		@Pc(16) ModelUnlit[] modelUnlits = new ModelUnlit[this.models.length];
		for (@Pc(18) int index = 0; index < this.models.length; index++) {
			modelUnlits[index] = ModelUnlit.get(Static14.aClass153_8, this.models[index]);
		}
		@Pc(56) ModelUnlit body;
		if (modelUnlits.length == 1) {
			body = modelUnlits[0];
		} else {
			body = new ModelUnlit(modelUnlits, modelUnlits.length);
		}
