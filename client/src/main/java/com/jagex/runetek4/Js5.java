...
public final class Js5 {
	...
	public boolean method4475() {
		...
		for (@Pc(22) int local22 = 0; local22 < this.index.anIntArray269.length; local22++) {
			@Pc(38) int local38 = this.index.anIntArray269[local22];
			if (this.packed[local38] == null) {
				this.fetchGroup(local38);
				if (this.packed[local38] == null) {
					local20 = false;
				}
			}
	...
	private boolean isIndexReady() {
		if (this.index == null) {
			...
			this.unpacked = new Object[this.index.anInt2907][];
			this.packed = new Object[this.index.anInt2907];
		}
		return true;
	}
	...
	private boolean unpackGroup(@OriginalArg(0) int group, @OriginalArg(2) int[] key) {
		if (!this.isGroupValid(group)) {
			...
			for (@Pc(55) int index = 0; index < local31; index++) {
				@Pc(62) int local62;
				if (local25 == null) {
					local62 = index;
				} else {
					local62 = local25[index];
				}
			...
			if (key == null || key[0] == 0 && key[1] == 0 && key[2] == 0 && key[3] == 0) {
				...
				local128.decryptXtea(key, local128.data.length);
			}
			@Pc(140) byte[] local140;
			try {
				local140 = Static162.method3092(local114);
			} catch (@Pc(142) RuntimeException local142) {
				System.out.println("T3 - " + (key != null) + "," + group + "," + local114.length + "," + Packet.getcrc(local114, local114.length) + "," + Packet.getcrc(local114, local114.length - 2) + "," + this.index.crcs[group] + "," + this.index.crc);
				local140 = new byte[] { 0 };
			}
	...
	private void fetchGroup(@OriginalArg(1) int group) {
		if (this.discardPacked) {
			...
			this.packed[group] = Static33.method869(this.provider.fetchgroup(group));
		}
	}
	...
	public int method4498() {
		...
		for (@Pc(22) int local22 = 0; local22 < this.packed.length; local22++) {
			if (this.index.groupSizes[local22] > 0) {
				local18 += 100;
	...
	public byte[] method4502(@OriginalArg(0) int file, @OriginalArg(1) int group) {
		if (!this.isGroupValid(group, file)) {
			return null;
		}
		if (this.unpacked[group] == null || this.unpacked[group][file] == null) {
		...
		return Static138.method2696(this.unpacked[group][file], false);
	}
