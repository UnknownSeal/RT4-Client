...
public final class VorbisSound extends Node {
	...
	public static void method2349(@OriginalArg(0) byte[] arg0) {
		...
		for (local17 = 0; local17 < 2; local17++) {
			...
			@Pc(193) int local193 = IntMath.bitCount(local38 - 1);
			for (@Pc(195) int local195 = 0; local195 < local38; local195++) {
				local187[local195] = IntMath.bitReverse(local193, local195);
			}
			if (local17 == 0) {
				trigA0 = local41;
				trigB0 = local86;
	...
	private float[] decodePacket(@OriginalArg(0) int arg0) {
		...
		readBit();
		@Pc(15) int local15 = readBits(IntMath.bitCount(modeMappings.length - 1));
		...
		if (local140) {
			...
			local272 = IntMath.bitCount(local25 - 1);
			@Pc(488) int local488;
			@Pc(499) int local499;
			@Pc(503) int local503;
			@Pc(505) int local505;
