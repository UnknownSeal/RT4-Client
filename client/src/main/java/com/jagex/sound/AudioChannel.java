...
import com.jagex.sound.core.Sound;
...
public class AudioChannel {
	...
	private long time = SystemTimer.safetime();
	...
	public static AudioChannel create(@OriginalArg(0) int arg0, @OriginalArg(1) SignLink arg1, @OriginalArg(2) Component arg2, @OriginalArg(3) int arg3) {
		...
		try {
			@Pc(33) AudioChannel local33 = (AudioChannel) Class.forName("com.jagex.sound.JavaAudioChannel").getDeclaredConstructor().newInstance();
	...
	private void read(@OriginalArg(0) int[] arg0) {
		...
		this.time = SystemTimer.safetime();
	}
	...
	public final synchronized void loop() {
		...
		@Pc(14) long now = SystemTimer.safetime();
		try {
	...
	public final synchronized void method3570() {
		...
		try {
			...
			this.flush();
			this.closeUntil = SystemTimer.safetime() + 2000L;
		}
	}
