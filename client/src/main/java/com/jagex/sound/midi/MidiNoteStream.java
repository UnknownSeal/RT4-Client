...
public final class MidiNoteStream extends PcmStream {
		...
		if ((this.parent.channelFlags[instrument.channel] & 0x4) != 0 && instrument.anInt3767 < 0) {
			...
			if (len >= local37) {
				if (this.parent.anIntArray509[instrument.channel] == 0) {
					...
					this.parent.method4442(instrument, instrument.instrument.aShortArray36[instrument.midiKey] < 0);
				}
				if (instrument.instrument.aShortArray36[instrument.midiKey] < 0) {
			...
			if (!this.parent.method4445(local17)) {
				...
				do {
					if (local29 <= local17.anInt3771) {
						this.read(arg0, local17, local31, local29, local31 + local29);
						local17.anInt3771 -= local29;
						break;
					}
					this.read(arg0, local17, local31, local17.anInt3771, local29 + local31);
	...
	private void read(@OriginalArg(0) int[] samples, @OriginalArg(1) MidiNote note, @OriginalArg(2) int offset, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4) {
		if ((this.parent.channelFlags[note.channel] & 0x4) != 0 && note.anInt3767 < 0) {
			@Pc(26) int local26 = this.parent.anIntArray503[note.channel] / AudioChannel.sampleRate;
			while (true) {
				@Pc(36) int local36 = (local26 + 1048575 - note.anInt3775) / local26;
				if (arg3 < local36) {
					note.anInt3775 += arg3 * local26;
					break;
				}
				arg3 -= local36;
				note.stream.read(samples, offset, local36);
				@Pc(55) int local55 = AudioChannel.sampleRate / 100;
				@Pc(58) SoundPcmStream local58 = note.stream;
				@Pc(62) int local62 = 262144 / local26;
				if (local62 < local55) {
					local55 = local62;
				}
				note.anInt3775 += local26 * local36 - 1048576;
				if (this.parent.anIntArray509[note.channel] == 0) {
					note.stream = SoundPcmStream.create(note.sound, local58.method405(), local58.getVolume(), local58.getPan());
				} else {
					note.stream = SoundPcmStream.create(note.sound, local58.method405(), 0, local58.getPan());
					this.parent.method4442(note, note.instrument.aShortArray36[note.midiKey] < 0);
					note.stream.method398(local55, local58.getVolume());
				}
				if (note.instrument.aShortArray36[note.midiKey] < 0) {
					note.stream.setLoops(-1);
				}
				offset += local36;
				local58.method384(local55);
				local58.read(samples, offset, arg4 - offset);
				if (local58.method412()) {
					this.mixer.addSubStream(local58);
				}
			}
		}
		note.stream.read(samples, offset, arg3);
	}
}