...
import com.jagex.sound.midi.MidiDecoder;
...
public final class Song extends Node {
	...
	public Song(@OriginalArg(0) Packet in) {
		...
		@Pc(500) Packet out = new Packet(this.midiBytes);
		out.p4(1297377380);
		out.p4(6);
		out.p2(tracks > 1 ? 1 : 0);
		out.p2(tracks);
		...
		label53: do {
			while (true) {
				...
				while (song.times[track] == local56) {
					song.loadTrackPosition(track);
					@Pc(69) int local69 = song.getNextEvent(track);
					if (local69 == 1) {
					...
					if (local85 == 176) {
						...
						if (local98 == 0) {
							banks[local92] = (banks[local92] & 0xFFE03FFF) + (local104 << 14);
						}
						if (local98 == 32) {
							banks[local92] = (banks[local92] & 0xFFFFC07F) + (local104 << 7);
						}
					}
					if (local85 == 192) {
						local92 = local69 & 0xF;
						local98 = local69 >> 8 & 0x7F;
						programs[local92] = banks[local92] + local98;
					}
					if (local85 == 144) {
						local92 = local69 & 0xF;
						local98 = local69 >> 8 & 0x7F;
